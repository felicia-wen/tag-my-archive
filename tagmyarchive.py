...
def Skip(t, *r):
    r = ''.join(map(str, r))
    print(Colors.FAINT+Colors.CYAN+"Skip:\t" +
          Colors.LIGHT_CYAN+t+Colors.LIGHT_WHITE+r+Colors.END)


def Info(t, *r):
    r = ''.join(map(str, r))
    print(Colors.BLUE+"Info:\t"+Colors.LIGHT_BLUE +
          t+Colors.LIGHT_PURPLE+r+Colors.END)


def Warn(t, *r):
    r = ''.join(map(str, r))
    print(Colors.NEGATIVE+Colors.BROWN+"Warning:\t" +
          Colors.YELLOW+t+Colors.LIGHT_WHITE+r+Colors.END)


def Debug(t, *r):
    r = ''.join(map(str, r))
    print(Colors.NEGATIVE+Colors.BOLD+Colors.BROWN+"Debug:\t" +
          Colors.YELLOW+t+Colors.LIGHT_WHITE+r+Colors.END)


def Error(t, *r):
    r = ''.join(map(str, r))
    print(Colors.NEGATIVE+Colors.RED+"Error:\t"+Colors.BLINK+t +
...
def start():
    ...
    if os.path.isdir(ext):
        ...
        os.makedirs(ext), os.chdir(ext)
    if os.name == 'nt':
        os.system(
            'echo $(which 7z unzip unrar | grep -E -o "7z$|unzip$|unrar$") > %ext%/available_ext')
    ...
    with open("available_ext", 'r') as avl:
        ...
        if '7z' in avlstr:
            ...
            if noask == 0:
                ask7z = filedialog.askopenfilename(
                    title="Where is your 7z.exe?", filetypes=[("EXE", ".exe")])
                p7z = ask7z.replace('"', "")
                path7z = r"{}".format(p7z)
    ...
    for root, dirs, files in os.walk(dlfolder):
        for name in files:
            ...
            if mv == 0:
                ...
                elif setpath == 1 or cuspath == 1:
                    Info("Windows:", "Extract using\t", path7z)
                elif p7zip == '7z':
            ...
            if mv == 1:
                ...
                Warn(
                    f"MvDir:\t--mvdir given,move directory:{fullpath} --> {extdir}")
            ...
            if setpath == 1 or cuspath == 1:
                os.environ['path7z'] = p7z
            edit = 1
            if os.name == 'nt':
                if _7z == 1:
                    osret = os.system(
                        '7z x "%fullpath%" -o"%extdir%" -y %arg7z%')
                if _un == 'zip':
                    osret = os.system(
                        'unzip "%fullpath%" -d "%extdir%" -o %argUz%')
                if _un == 'rar':
                    osret = os.system(
                        'unrar x "%fullpath%" "%extdir%" y %argUr%')
                if setpath == 1 or cuspath == 1:
                    osret = os.system(
                        '""%path7z%"" x "%fullpath%" -o"%extdir%" -y %arg7z%')
                os.system('chmod -R 775 "%extdir%"')
            else:
                if _7z == 1:
                    osret = os.system('7z x "$fullpath" -o"$extdir" -y $arg7z')
                if _un == 'zip':
                    osret = os.system(
                        'unzip "$fullpath" -d "$extdir" -o $argUz')
                if _un == 'rar':
                    osret = os.system('unrar x "$fullpath" "$extdir" y $argUr')
                if setpath == 1 or cuspath == 1:
                    osret = os.system(
                        '"$path7z" x "$fullpath" -o"$extdir" -y $arg7z')
                os.system('chmod -R 775 "$extdir"')
            if osret == 0:
                ...
                Error("Encountered with error:", str(osret))
            f.close
    if osret == 0 and cuspath == 1:
        cusf = open("7z.path", "x")
        cusf.write(path7z)
        cusf.close
    if os.path.isfile('available_ext'):
...
def CheckDir(extdir):
    if os.path.isdir(extdir):
        ...
        os.makedirs(extdir), Info("mkdir:", extdir)
...
def Matches(fname):
    ...
    if _case == 0:
        ...
        if len(e1) == 2:
            ...
            Info("Using filter:", 'Space')
            e = e2
            n1 = e[0]
            n2 = e[1]
            _match = 1
        elif len(e3) == 2:
            Info("Using filter:", 'Dot')
            e = e3
            n1 = e[0]
            n2 = e[1]
            _match = 1
...
try:
    options, otheropts = getopt.getopt(
        sys.argv[1:], "sx:o:h", ['noask', 'mvdir', 'exec7z=', 'execUr=', 'execUz='])
...
for option, argument in options:
    ...
    if option == '--noask':
        ...
        noask = 1
    if option == '--mvdir':
        Warn("\t--mvdir is given,this may cause system impact.")
        mvdir = 1
    if option == '-s':
        sta = 1
    if option == '--exec7z':
        os.environ['arg7z'] = argument
    if option == '--execUr':
        os.environ['argUr'] = argument
    if option == '--execUz':
        os.environ['argUz'] = argument
    else:
        os.environ['arg'] = ''
